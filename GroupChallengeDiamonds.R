...
statsDiamonds <- diamonds %>%
  ...
  select(c("x","y","z")) %>%
  summarise(
    min = min(diamonds$x, na.rm = TRUE),
    Q1 = quantile(diamonds$x, probs = 0.25, na.rm = TRUE),
    median = median(diamonds$x, na.rm = TRUE),
    Q3 = quantile(diamonds$x, probs = 0.75, na.rm = TRUE),
    max= max(diamonds$x, na.rm = TRUE)
  )
View(statsDiamonds)
...
statsDiamonds <- diamonds %>%
  ...
  summarise(
    ...
    standard_deviation_z = sd(z, na.rm = TRUE)
  )
View(statsDiamonds)